...
export function CreateDevisModal({ onClose, defaultClientId }: Props) {
  ...
  function pickProduit(index: number, produit: Produit) {
    setLignes((prev) =>
      prev.map((l, i) =>
        i === index ? { ...l, designation: produit.nom, prix_unitaire: Number(produit.prix_unitaire), tva_taux: Number(produit.tva_taux) } : l
      )
    );
  ...
  function handleSubmit() {
    ...
    setFormError('');
    createDevis(
      { client_id: clientId, date_emission: dateEmission, date_validite: dateValidite || undefined, notes: notes || undefined, lignes },
      {
        onSuccess: onClose,
  ...
  return (
    <Modal title="Nouveau devis" onClose={onClose} size="lg">
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input label="Date de validité" type="date" value={dateValidite} onChange={(e) => setDateValidite(e.target.value)} />
        </div>
              ...
              return (
                <div key={i} className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-5 flex gap-1 items-center relative">
                    ...
                    <input className="flex-1 min-w-0 border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
                      placeholder="Saisie libre..." value={ligne.designation}
                      onChange={(e) => updateLigne(i, 'designation', e.target.value)} />
                  </div>
                  <input type="number" className="col-span-2 border border-gray-300 rounded-lg px-2 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-green-500"
                    value={ligne.quantite} min={1} onChange={(e) => updateLigne(i, 'quantite', parseFloat(e.target.value) || 1)} />
                  <input type="number" className="col-span-2 border border-gray-300 rounded-lg px-2 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-green-500"
                    placeholder="0" value={ligne.prix_unitaire || ''} min={0}
                    onChange={(e) => updateLigne(i, 'prix_unitaire', parseFloat(e.target.value) || 0)} />
                  <input type="number" className="col-span-1 border border-gray-300 rounded-lg px-2 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-green-500"
                    value={ligne.tva_taux} min={0} onChange={(e) => updateLigne(i, 'tva_taux', parseFloat(e.target.value) || 0)} />
                  <span className="col-span-1 text-right text-xs text-gray-600 font-medium tabular-nums">{formatFCFA(ttc)}</span>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
              ...
              rows={2} placeholder="Conditions, remarques..." />
          </div>
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Button className="flex-1 justify-center" loading={isPending} onClick={handleSubmit}>Créer le devis</Button>
        </div>
      </div>
    </Modal>
  );
